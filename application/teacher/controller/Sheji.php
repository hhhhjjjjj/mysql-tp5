...
class Sheji extends Base
{
    public function sheji(){
        ...
        $list=db()->table('keti_list')
            ...
            ->field('a.id,a.keti_name,a.origin,a.teacher,a.jiaoyan,a.detail,a.shejiS_state,a.gonghao,a.xuehao,b.sheji_state')
            ->paginate(5,false,[]);
    ...
    public function shenhe(){
        if(request()->isPost()){
            ...
            $data_se['shejiS_state']=$data['state'];
            $bool1=db()->table('sheji_list')->where('xuehao',$user_xuehao)->update($data);
            ...
            // $id=input('id');
            $data=db()->table('sheji_list')->where('xuehao',$xuehao)->find();
            $info['beijing']=$data['beijing'];
            $info['fenxi']=$data['fenxi'];
            $info['zongjie']=$data['zongjie'];
