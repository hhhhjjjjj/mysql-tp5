...
class Check extends Base
{
    ...
    if($_FILES['file_path']['size'] > $size ) {
        ...
        $list=db()->table('check_list')->where("xuehao",$xuehao)->select();
        $find_one=db()->table('kaiti_list')->where("xuehao",$xuehao)->find();
        if($find_one['state']==1){
        $count=count($list);
        if($count==1){
            $state=$list[0]['state'];
            if($state==0){
                $data['timer']=time();
                $bool=db()->table('check_list')->where("xuehao",$xuehao)->update($data);
                if($bool){
                    $data_ren['check_state']=1;
                    $bool_ren=db()->table('user_study')->where("xuehao",$xuehao)->update($data_ren);
                    $bool_file=db()->table('check_list')->where("xuehao",$xuehao)->update($data_kt);
                    if($bool_file){
                        $this->success('提交中期检查成功');
                    }   
                }             
            }elseif($state==1){
                $this->error('提交中期检查失败,中期检查通过审核');
            }
            
        }else{
             $data['timer']=time();
            $bool=db()->table('check_list')->insert($data);
            $bool_file=db()->table('check_list')->where("xuehao",$xuehao)->update($data_kt);
            if($bool){
                $data_ren['check_state']=1;
                ...
                // dump($data_ss);
                // exit;
                 if($data_ss['check_state']==1){
                     $this->error('提交中期检查失败,你已经提交过中期检查');
                 }else{
                    $bool_ren=db()->table('user_study')->where("xuehao",$xuehao)->update($data_ren);
                    if($bool_ren){
                        $this->success('提交中期检查成功');
            ...
            $this->error('提交中期检查失败,您的开题报告还没通过审核');
        }

        }else{
            $xuehao=session('user_xuehao');
             $list_ren=db()->table('check_list')->where("xuehao",$xuehao)->select();
        ...
        if($count==1){
            ...
            $info_ren['user']=session('user_name');
             ...
             $info_ren['check_state']=$info_stu['check_state'];
            //  dump($info_ren);
            //  exit;
    ...
    public function see(){
        $xuehao=session('user_xuehao');
          $info=db()->table('check_list')
            ->alias('a')
            ->join('keti_list b','a.xuehao=b.xuehao')
            ->join('user_study c','b.xuehao=c.xuehao')
            ->where('a.xuehao',$xuehao)
            ->where('c.state',0)
            ->field('c.xuehao,c.user,c.class,b.keti_name,b.origin,b.detail,a.wancheng,a.weicheng,a.timer,a.state,a.file_path,a.file_name,a.file_size,a.pingjia')
            ->find();
    ...
    $xuehao=input('xuehao');
    $file_n=db()->table('check_list')->where('xuehao',$xuehao)->find();
    if(!$file_n){
        return "暂无下载入口";
    }
