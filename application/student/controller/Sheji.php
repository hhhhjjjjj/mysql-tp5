...
class Sheji extends Base
{
    ...
    if($_FILES['file_path']['size'] > $size ) {
        ...
        $list=db()->table('sheji_list')->where("xuehao",$xuehao)->select();
        $find_one=db()->table('check_list')->where("xuehao",$xuehao)->find();
        if($find_one['state']==1){
        $count=count($list);
        if($count==1){
            $state=$list[0]['state'];
            if($state==0){
                $data['timer']=time();
                $bool=db()->table('sheji_list')->where("xuehao",$xuehao)->update($data);
                if($bool){
                    $data_ren['sheji_state']=1;
                ...
                $this->error('提交毕业设计失败,毕业设计通过审核');
            }
            
        }else{
             $data['timer']=time();
            $bool=db()->table('sheji_list')->insert($data);
            $bool_file=db()->table('sheji_list')->where("xuehao",$xuehao)->update($data_kt);
            if($bool){
                $data_ren['sheji_state']=1;
                $data_ss=db()->table('user_study')->where("xuehao",$xuehao)->find();
            ...
            $this->error('提交毕业设计失败,您的中期检查还没通过审核');
        }

        }else{
            $xuehao=session('user_xuehao');
             $list_ren=db()->table('sheji_list')->where("xuehao",$xuehao)->select();
        ...
        if($count==1){
            ...
            $info_ren['user']=session('user_name');
             ...
             $info_ren['sheji_state']=$info_stu['sheji_state'];
            //  dump($info_ren);
            //  exit;
    ...
    public function see(){
        $xuehao=session('user_xuehao');
        $info=db()->table('sheji_list')
            ->alias('a')
            ->join('keti_list b','a.xuehao=b.xuehao')
            ->join('user_study c','b.xuehao=c.xuehao')
            ->where('a.xuehao',$xuehao)
            ->where('c.state',0)
            ->field('c.xuehao,c.user,c.class,b.keti_name,b.origin,b.detail,a.beijing,a.fenxi,a.zongjie,a.timer,a.state,a.file_path,a.file_name,a.file_size,a.pingjia')
            ->find();
    ...
    $xuehao=input('xuehao');
    $file_n=db()->table('sheji_list')->where('xuehao',$xuehao)->find();
    if(!$file_n){
        return "暂无下载入口";
    }
