...
class Kaiti extends Base
{
    ...
    if($_FILES['file_path']['size'] > $size ) {
        ...
        $list=db()->table('kaiti_list')->where("xuehao",$xuehao)->select();
        $find_one=db()->table('renwu_list')->where("xuehao",$xuehao)->find();
        if($find_one['state']==1){
        $count=count($list);
        if($count==1){
            $state=$list[0]['state'];
            if($state==0){
                $data['timer']=time();
                $bool=db()->table('kaiti_list')->where("xuehao",$xuehao)->update($data);
                if($bool){
                    $data_ren['kaiti_state']=1;
                    $bool_ren=db()->table('user_study')->where("xuehao",$xuehao)->update($data_ren);
                    $bool_file=db()->table('kaiti_list')->where("xuehao",$xuehao)->update($data_kt);
                    if($bool_file){
                        $this->success('提交开题报告成功');
                    }   
                }             
            }elseif($state==1){
                $this->error('提交开题报告失败,开题报告通过审核');
            }
            ...
            //  dump($data);
            //  exit;
            $bool=db()->table('kaiti_list')->insert($data);
            $bool_file=db()->table('kaiti_list')->where("xuehao",$xuehao)->update($data_kt);
            if($bool){
                $data_ren['kaiti_state']=1;
                 $data_ss=db()->table('user_study')->where("xuehao",$xuehao)->find();
                 if($data_ss['kaiti_state']==1){
                     $this->error('提交开题报告失败,你已经提交过开题报告');
                 }else{
                    $bool_ren=db()->table('user_study')->where("xuehao",$xuehao)->update($data_ren);
                    if($bool_ren){
                        $this->success('提交开题报告成功');
            ...
            $this->error('提交开题报告失败,您的任务书还没通过审核');
        }

        }else{
            $xuehao=session('user_xuehao');
             $list_ren=db()->table('kaiti_list')->where("xuehao",$xuehao)->select();
            ...
            if($count==1){
                ...
                $info_ren['kaiti_state']=$info_stu['kaiti_state'];
                //  dump($info_ren);
                //  exit;
    ...
    public function see(){
        $xuehao=session('user_xuehao');
        $info=db()->table('kaiti_list')
            ->alias('a')
            ->join('keti_list b','a.xuehao=b.xuehao')
            ->join('user_study c','b.xuehao=c.xuehao')
            ->where('a.xuehao',$xuehao)
            ->where('c.state',0)
            ->field('c.xuehao,c.user,c.class,b.keti_name,b.origin,b.detail,a.yiyi,a.jiegou,a.fangfa,a.timer,a.state,a.file_path,a.file_name,a.file_size,a.pingjia')
            ->find();
